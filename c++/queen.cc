...
{
public:
    QueenProblemSolver(int N_) : N{N_}
    {
        if (N_ <= 0) return;
        placed_queen_id.resize(N, 0);
        init_occupancy();
...
private:
    void init_occupancy()
    {
        ...
        is_occupied_right_diagonal.resize(
            get_number_of_diagonals(),
            OccupancyStatus::FREE);
        is_occupied_left_diagonal.resize(
            get_number_of_diagonals(),
            OccupancyStatus::FREE);
    }
    ...
    {
        return is_occupied_column[column] == OccupancyStatus::OCCUPIED
